...
| 风险 | 影响 | 处理方向 |
|---|---|---|
| Provider 与 Room 双写/双读边界不清晰 | 列表、编辑、同步可能看到不同数据 | 明确迁移阶段的数据权威来源，并补齐桥接逻辑。 |
| `allowMainThreadQueries()` 仍存在 | 可能造成主线程阻塞 | 完成异步 DAO 调用后移除。 |
| Room 数据库与 legacy 数据库迁移未闭环 | 升级路径可能丢数据或重复数据 | 设计一次性迁移和回滚策略。 |
| 小组件 / 闹钟 / 搜索仍依赖旧模型 | Room-only 改动会造成回归 | 迁移前建立功能清单和验收用例。 |
| 非 WebDAV 流程测试不足 | 回归只能靠手动验证 | 按风险补单元测试和少量 instrumentation 测试。 |
